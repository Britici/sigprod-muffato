...
function verDet(id,tipo) {
  ...
  if(tipo==='os'){
    rows+=`
      ...
      ${item.fotoUrl?`<div class="dr" style="flex-direction:column;gap:8px"><span class="dl">📷 Foto</span><span class="dv"><a href="${item.fotoUrl}" target="_blank"><img src="${driveThumb(item.fotoUrl)}" style="max-width:100%;max-height:220px;border-radius:var(--rs);object-fit:contain;border:1px solid var(--bord);cursor:zoom-in" alt="Foto OS"></a></span></div>`:''}      ${item.origem!=='direta'?`<div class="dr"><span class="dl">OS Origem</span><span class="dv" style="color:var(--red)">${item.origemNum}</span></div>`:''}`;
